...
const CHECKLIST_SECTIONS: ChecklistSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'db-vacuum', label: 'VACUUM ANALYZE run on core tables' },
    ],
  },
  {
    id: 'environment',
    ...
    items: [
      { id: 'env-vite-url', label: 'VITE_SUPABASE_URL configured', critical: true },
      { id: 'env-vite-key', label: 'VITE_SUPABASE_ANON_KEY configured', critical: true },
      { id: 'env-vite-app', label: 'VITE_APP_URL set to production domain' },
      { id: 'env-vite-stripe', label: 'VITE_STRIPE_PUBLISHABLE_KEY (live key)' },
      { id: 'env-twilio-sid', label: 'TWILIO_ACCOUNT_SID in Supabase Secrets', critical: true },
      { id: 'env-twilio-token', label: 'TWILIO_AUTH_TOKEN in Supabase Secrets', critical: true },
      { id: 'env-resend', label: 'RESEND_API_KEY in Supabase Secrets', critical: true },
      { id: 'env-stripe-secret', label: 'STRIPE_SECRET_KEY (live) in Supabase Secrets', critical: true },
      { id: 'env-stripe-webhook', label: 'STRIPE_WEBHOOK_SECRET in Supabase Secrets', critical: true },
      { id: 'env-stripe-connect', label: 'STRIPE_CONNECT_WEBHOOK_SECRET configured' },
      { id: 'env-stripe-prices', label: 'STRIPE_STARTER_PRICE_ID & STRIPE_GROWTH_PRICE_ID set' },
      { id: 'env-platform-fee', label: 'STRIPE_PLATFORM_FEE_PERCENT configured' },
      { id: 'env-app-url', label: 'APP_URL in Supabase Secrets' },
    ],
  },
  {
    id: 'domain',
    ...
    items: [
      { id: 'domain-registered', label: 'Production domain registered' },
      { id: 'domain-ssl', label: 'SSL certificate active', critical: true },
      { id: 'domain-dns', label: 'DNS configured (app.selestial.io → hosting)' },
      { id: 'domain-email-spf', label: 'Email SPF record configured' },
      { id: 'domain-email-dkim', label: 'Email DKIM record configured' },
      { id: 'domain-email-dmarc', label: 'Email DMARC record configured' },
    ],
  },
  {
    id: 'email',
    ...
    items: [
      ...
      { id: 'email-domain', label: 'Sending domain verified in Resend' },
      { id: 'email-test-gmail', label: 'Test emails arrive in Gmail inbox' },
      { id: 'email-test-outlook', label: 'Test emails arrive in Outlook inbox' },
      { id: 'email-spam-score', label: 'Spam score 9/10 or higher (mail-tester.com)' },
      { id: 'email-template', label: 'Email templates render correctly' },
      { id: 'email-links', label: 'All links in emails work' },
      { id: 'email-mobile', label: 'Emails render on mobile' },
    ],
  },
  {
    id: 'monitoring',
    ...
    items: [
      { id: 'mon-sentry', label: 'Error monitoring configured (Sentry)' },
      { id: 'mon-uptime', label: 'Uptime monitoring active' },
      { id: 'mon-uptime-api', label: 'API endpoint monitoring' },
      { id: 'mon-cron', label: 'Cron job monitoring (process-queue)' },
      { id: 'mon-analytics', label: 'Product analytics configured (PostHog)' },
      { id: 'mon-alerts', label: 'Alert notifications configured' },
    ],
  },
  {
    id: 'stripe-config',
    ...
    icon: <CreditCard className="h-4 w-4" />,
    items: [
      { id: 'stripe-live', label: 'Stripe in live mode', critical: true },
      { id: 'stripe-products', label: 'Starter & Growth products created' },
      { id: 'stripe-webhook-billing', label: 'Billing webhook endpoint configured' },
      { id: 'stripe-webhook-connect', label: 'Connect webhook endpoint configured' },
      { id: 'stripe-connect-brand', label: 'Connect branding configured' },
      { id: 'stripe-connect-redirect', label: 'Connect redirect URLs set to production' },
    ],
  },
  {
    id: 'twilio-config',
    ...
    icon: <MessageSquare className="h-4 w-4" />,
    items: [
      { id: 'twilio-upgrade', label: 'Account upgraded from trial', critical: true },
      { id: 'twilio-numbers', label: 'Production phone numbers purchased' },
      { id: 'twilio-messaging', label: 'Messaging service configured' },
      { id: 'twilio-webhooks', label: 'Webhook URLs set to production' },
    ],
  },
  {
    id: 'legal',
    ...
    items: [
      { id: 'legal-terms', label: 'Terms of Service page published', critical: true },
      { id: 'legal-privacy', label: 'Privacy Policy page published', critical: true },
      { id: 'legal-cookie', label: 'Cookie Policy (if using cookies)' },
      { id: 'legal-acceptable', label: 'Acceptable Use Policy' },
      { id: 'legal-tcpa', label: 'SMS compliance (TCPA) disclosed' },
      { id: 'legal-consent', label: 'User consent for messaging in onboarding' },
      { id: 'legal-pricing', label: 'Clear pricing displayed before purchase' },
      { id: 'legal-refund', label: 'Refund policy stated' },
    ],
  },
  {
    id: 'support',
    ...
    items: [
      { id: 'support-docs', label: 'Help documentation created' },
      { id: 'support-faq', label: 'FAQ section available' },
      { id: 'support-email', label: 'Support email configured (support@selestial.io)' },
      { id: 'support-auto', label: 'Auto-responder configured' },
      { id: 'support-inapp', label: 'In-app help button/link' },
    ],
  },
  {
    id: 'go-live',
    ...
    items: [
      ...
      { id: 'live-migrations', label: 'All migrations applied' },
      { id: 'live-indexes', label: 'Indexes verified' },
      { id: 'live-rls', label: 'RLS verified' },
      ...
      { id: 'live-monitoring', label: 'Error monitoring active' },
      { id: 'live-analytics', label: 'Analytics configured' },
      { id: 'live-terms', label: 'Terms of Service published' },
      { id: 'live-privacy', label: 'Privacy Policy published' },
  ...
  },
  {
    ...
    items: [
      { id: 'final-signup', label: 'Can sign up (production)', critical: true },
      { id: 'final-onboard', label: 'Can complete onboarding', critical: true },
      { id: 'final-quote', label: 'Can create quote' },
      { id: 'final-sms', label: 'SMS sends successfully', critical: true },
      { id: 'final-email', label: 'Email sends successfully' },
      { id: 'final-payment', label: 'Payment works', critical: true },
      { id: 'final-e2e', label: 'Full end-to-end test passed' },
    ],
  },
  {
    id: 'post-launch',
    ...
    items: [
      { id: 'post-errors', label: 'Monitor error rate (first 24h)' },
      { id: 'post-signups', label: 'Track signup success rate' },
      { id: 'post-onboard', label: 'Track onboarding completion rate' },
      { id: 'post-sms-delivery', label: 'Monitor SMS delivery rate' },
      { id: 'post-email-delivery', label: 'Monitor email delivery rate' },
      { id: 'post-payment', label: 'Monitor payment success rate' },
      { id: 'post-support', label: 'Address support tickets' },
      { id: 'post-costs', label: 'Review costs (Twilio, Stripe, Supabase)' },
    ],
  },
];
