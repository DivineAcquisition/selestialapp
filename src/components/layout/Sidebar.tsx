...
export default function Sidebar() {
  ...
  const NavLink = ({ item }: { item: typeof navigation[0] }) => {
    ...
    return (
      <Link
        href={item.href}
        className={cn(
          'group flex items-center justify-between gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200',
          isActive
      ...
      >
        ...
        {item.showBadge && totalUnread > 0 && (
          <span className="flex items-center justify-center min-w-[18px] h-5 px-1.5 text-[10px] font-bold bg-destructive text-destructive-foreground rounded-full animate-pulse">
  ...
  return (
    <div className="flex h-screen w-60 flex-col bg-sidebar-background border-r border-sidebar-border">
      {/* Logo */}
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary transition-colors cursor-pointer group">
          <div className="relative">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-primary-foreground text-sm font-semibold glow-sm">
              {initials}
            </div>
